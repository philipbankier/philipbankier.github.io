...
Personal website for Philip Bankier. Built with React 19 + Tailwind CSS 4 + Vite, deployed as a static site on GitHub Pages.
...
Pushing to `main` triggers `.github/workflows/deploy.yml`, which runs `pnpm run build:gh` and publishes `dist/public/` to GitHub Pages (custom domain `philipbankier.com`).
...
Edit the `PERSONAL_POSTS` and `ACTRUN_POSTS` arrays in:
```
client/src/components/WritingSection.tsx
```

### Adding/Editing Library Artifacts
Drop the artifact HTML into `client/public/pages/`, then add an entry to the `ARTIFACTS` array in:
```
client/src/components/LibrarySection.tsx
```
